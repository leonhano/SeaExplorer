...
static public class Tools
{
    ...
    static public bool GetTouchPositionInGame(int layerMask, out Vector3 touchPosInGame)
    {

        Vector2 touchPos = Vector2.zero;
        touchPosInGame = Vector3.zero;
        bool bGetPos = false;

        if (Camera.main == null)    //main camera is not active
            return false;
        ...
        {
            touchPosInGame = hit.point;
            Debug.Log("touchPosInGame = " + touchPosInGame);
            return true;
        }
        return false;
    }

    static public bool GetTouchPositionInWater(out Vector3 touchPosInGame)
    {
        int raycastMask = (1 << LayerMask.NameToLayer("Water"));             //raycastMask = ~raycastMask;
        return GetTouchPositionInGame(raycastMask, out touchPosInGame);
    }
    ...
    static public Quaternion LookAtPlayerOnYAxis(Vector3 srcPos, Vector3 targetPos)
    {
        var rotation = Quaternion.LookRotation(targetPos - srcPos).eulerAngles;
        rotation.x = 0;
        rotation.z = 0;
